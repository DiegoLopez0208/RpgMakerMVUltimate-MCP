...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        mode: { type: 'string', enum: ['blank', 'themed', 'procedural', 'batch', 'duplicate', 'template', 'semantic'], description: 'Which generator to use; see the tool description. Default "procedural"' },
        ...
        rooms: { ...ID_TYPE, description: 'mode "semantic": rooms on the critical path (default 5)' },
        sideRooms: { ...ID_TYPE, description: 'mode "semantic": dead-end rooms hung off the path, the first holding treasure (default 2)' },
        locked: { type: 'boolean', description: 'mode "semantic": put a key and a locked door on the path (default true when there are 4+ rooms)' },
        loop: { type: 'boolean', description: 'mode "semantic": add one extra corridor so the map has a loop instead of being a tree (default true)' },
        keepProps: { type: 'boolean', description: 'mode "semantic" with a mined templateId: paint the original decorations even onto a different tileset (they may not exist there)' },
  ...
  {
    name: 'manage_system',
    description: 'Read or edit project-wide settings in data/System.json (writes are immediate). action "get" returns the requested `section`: "full" (everything — large), "switches" or "variables" (name arrays indexed by ID; unnamed entries are empty strings — use these to find free IDs), or "title". action "set_title" changes the game title shown on the title screen. "name_switch"/"name_variable" label a switch/variable by ID — documentation only, runtime values are untouched, but good names keep event logic readable. "set_starting_position" sets where new games begin {mapId, x, y} — NOT validated against existing maps, verify with query_map "infos" first; does not affect saved games. "create_plugin" authors a new plugin: it writes js/plugins/<name>.js with a correct @plugindesc/@author/@param/@help header (and a classic-MV Game_Interpreter.pluginCommand hook when `commands` are given) and registers it in js/plugins.js (array order = load order). Re-authoring the same `name` overwrites the file and replaces its manifest entry in place. Pair with analyze_project view "plugins" to inspect the project\'s existing plugins first. "scaffold_project" creates a NEW, separate RPG Maker MV project by cloning the engine\'s blank template (NewData) into `destPath` and rewriting its title (`title`) and start position (`mapId`/`x`/`y`); the source install is `sourcePath` or the RPGMAKER_MV_INSTALL env var or the default Steam location, and it refuses to overwrite a directory that already holds a project. This does NOT switch the active project — use set_project_path afterwards. "playtest" launches the ACTIVE project through the engine\'s bundled nwjs runtime (like the editor\'s Playtest button) so you can actually see a change running — a game window opens (the user closes it); returns the launched process info. "open_editor" opens the active project in the RPGMV.exe editor (best-effort). Both locate the engine via `install` / RPGMAKER_MV_INSTALL / the default Steam path and are Windows-only. THE LIVE BRIDGE closes the loop between editing and seeing, because playtest on its own tells you nothing about what the game did. "install_bridge_plugin" writes js/plugins/McpBridge.js (once per project; it only activates in playtest, never in a deployed build). "bridge_start" opens a loopback WebSocket and writes the port and session token to .mcp-bridge.json; then launch "playtest" and the game connects on its own. "bridge_status" reports whether the game is connected. "bridge_telemetry" drains what the game reported — exceptions, console errors, scene changes, player position, which event command is executing — optionally filtered by `types` and capped by `limit`; frames are removed as you read them unless `peek` is true. "bridge_command" sends ONE instruction named by `command`: "reload_map" re-reads the current MapXXX.json from disk and rebuilds the scene without losing party state (use it right after edit_map to see the change), "reload_database" re-reads one data file named by `file` (Actors/Classes/Skills/Items/Weapons/Armors/Enemies/Troops/States/CommonEvents .json — System.json and Tilesets.json need a fresh playtest), "get_state" dumps the switches and variables currently set, "teleport_player" moves the player to `x`/`y` (and `mapId`), "ping" checks liveness. "bridge_screenshot" captures the game window and saves a PNG, returning its path for analyze_image. "bridge_stop" closes the socket and deletes the handshake file. LEARNING FROM THE PROJECT: "mine_templates" reads every map in THIS project and derives three things into .mcp-cache/ — semantic layouts with the art stripped out, a profile per tileset saying which concrete tile the project uses for ground/wall/water/roof, and how often each kind of cell sits next to which. Nothing in the project is modified. Run it once before generate_map mode "semantic", which needs the profile to know how to paint a layout with the tiles THIS project uses (including third-party or DLC tilesets the bundled RTP templates cannot target). `minDistinctTiles` (default 10) is how much variety a map needs before it counts as content rather than scratch; `limit` caps how many layouts are kept, largest first. Returns the counts, the skipped maps with reasons, and the profiles it learned. Returns the read section or the updated values.',
    ...
    inputSchema: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['get', 'set_title', 'name_switch', 'name_variable', 'set_starting_position', 'create_plugin', 'scaffold_project', 'playtest', 'open_editor', 'mine_templates', 'install_bridge_plugin', 'bridge_start', 'bridge_stop', 'bridge_status', 'bridge_telemetry', 'bridge_command', 'bridge_screenshot'], description: 'What to do; see the tool description. Default "get"' },
        ...
        peek: { type: 'boolean', description: 'bridge_telemetry: leave the frames in the buffer instead of consuming them' },
        wait: { type: 'boolean', description: 'bridge_command: wait for the game to answer (default true; false is fire-and-forget)' },
        timeoutMs: { ...ID_TYPE, description: 'bridge_command/bridge_screenshot: how long to wait for the answer (default 8000, screenshots 15000)' },
  ...
  },
  {
    ...
    description: 'Read-only project intelligence: builds (and caches) an in-memory model of the WHOLE project so you can reason about it instead of re-reading files. `view` selects the lens: "overview" (default) returns game title, entity/map/event counts, a health summary (errors/warnings/info) with the top issues, and any maps unreachable from the start map — the fastest way to understand a project you did not build; "index" returns the structured digest (every map with its event count, common events, and only the NAMED switches/variables); "validate" runs every consistency check (broken transfers to non-existent maps, MapInfos entries whose Map file is missing, events that call missing common events / items / weapons / armors / troops / animations, duplicate database IDs, named-but-unused switches/variables, bad starting position, unreachable maps) returning {issueCount, bySeverity, issues[]}, optionally filtered by `severity`; "graph" returns the map transfer network (nodes, directed edges) plus reachability from the start map; "usage" answers "what uses X?" — pass `kind` (switch/variable/common_event/item/weapon/armor/troop/animation/actor/state/map) and `id` to get every event, common event and troop that references it, with read/write roles for switches/variables; "explain" reasons about one thing — `target` "switch"/"variable" + `id` tells you whether it is set, read, gated, a dead write, or never-set (the usual reason a door/event never triggers), and `target` "map" + `id` reports incoming transfers, what becomes unreachable if it is deleted, and whether it is the start map; "ast" parses one event page (mapId + eventId, optional page) or a common event (commonEventId) into a logical tree with a readable outline; "plugins" fuses js/plugins.js with each plugin file\'s @plugindesc/@author/@param/@command/@help header so you can adapt to the project\'s OWN systems instead of emitting vanilla events (and flags configured plugins whose file is missing); "critique" reviews ONE map (`mapId`) like a game designer — dead space, empty/cluttered balance, event distribution across quadrants, floor monotony, fragmented walkable regions — returning metrics plus justified, actionable suggestions and a rough score; "metrics" measures ONE map (`mapId`) instead of judging it: flood-fill reachability from the real entry point (walkable tiles the player can never get to, and events with no reachable tile beside them — these are softlocks, not style notes), dead-space ratio against the band for `expected` ("interior"/"dungeon"/"exterior"), the walkable area thinned to a skeleton and read as a graph (endpoints, junctions, loops, critical path, linearity — linearity near 1 is a bare corridor), Shannon entropy over 5x5 tile windows for visual monotony, and, when the map has random encounters, how many steps the player is from a shop/inn/save point. Use it to check a generated map before shipping it, and to compare two maps objectively; "critique" is the opinion, "metrics" is the measurement. "balance" compares every database entry against its PEERS instead of against invented thresholds, so it works on a game at any power level: damage per MP for skills, gold per point of ATK+MAT for weapons, gold per point of DEF+MDF for armors, HP per point of EXP for enemies. The comparison leaves each entry out of its own statistics, which is what keeps a badly broken entry from dragging the mean far enough to hide itself. Filter with `category` (skills/weapons/armors/enemies/all) and loosen or tighten with `thresholdSd` (default 2 standard deviations). Damage formulas are parsed, never executed, and one that cannot be read statically is listed under `unreadableFormulas` rather than counted as zero damage. Each outlier says what it means for the player, not just how far out it sits; "refactor" finds command sequences copy-pasted across events/common events and suggests extracting them into a Common Event; "search" ranks the project\'s human-readable text (map/NPC names, dialogue, item/skill descriptions, notes) against a free-text `query` like "the blacksmith" or "the dark forest". Nothing is written. Pair with the editor tools to act on what you find.',
    annotations: { title: 'Analyze project', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    inputSchema: {
      type: 'object',
      properties: {
        view: { type: 'string', enum: ['overview', 'index', 'validate', 'graph', 'usage', 'explain', 'ast', 'plugins', 'critique', 'metrics', 'balance', 'refactor', 'search'], description: 'Which lens to apply (default "overview")' },
        category: { type: 'string', enum: ['skills', 'weapons', 'armors', 'enemies', 'all'], description: 'view "balance": analyse one category instead of all four (default "all")' },
        thresholdSd: { ...ID_TYPE, description: 'view "balance": how many standard deviations from its peers an entry must sit before it is flagged (default 2; raise to see only the worst)' },
        expected: { type: 'string', enum: ['interior', 'dungeon', 'exterior'], description: 'view "metrics": what kind of space this map is meant to be, which sets the dead-space band (default "exterior")' },
