...
var TEMPLATE_INDEX = null;

function loadTemplateIndex() {
  if (TEMPLATE_INDEX) return TEMPLATE_INDEX;
  var idxPath = path.join(import.meta.dirname, "..", "knowledge", "map-templates.json");
  if (fs.existsSync(idxPath)) {
    TEMPLATE_INDEX = JSON.parse(fs.readFileSync(idxPath, "utf8"));
  } else { TEMPLATE_INDEX = []; }
  return TEMPLATE_INDEX;
}

function searchTemplates(category, theme) {
  var idx = loadTemplateIndex();
...
function generateFromTemplate(templateId, opts) {
  var fn = "Map" + String(templateId).padStart(3, "0") + ".json";
  var fp = path.join(import.meta.dirname, "..", "knowledge", "maps", fn);
  if (!fs.existsSync(fp)) return null;
  var map = JSON.parse(fs.readFileSync(fp, "utf8"));
  var w = (opts && opts.width) || map.width;
  var h = (opts && opts.height) || map.height;
  if (w === map.width && h === map.height) {
    return { data: map.data.slice(), width: w, height: h, events: (opts && opts.keepEvents) ? map.events : [] };
  }
  var data = new Array(w * h * 6).fill(0);
  for (var layer = 0; layer < 6; layer++) {
    for (var y = 0; y < Math.min(h, map.height); y++) {
      for (var x = 0; x < Math.min(w, map.width); x++) {
        var srcIdx = (layer * map.height + y) * map.width + x;
        var dstIdx = (layer * h + y) * w + x;
...
function generateMap(opts) {
  var method = (opts && opts.method) || "procedural";
  if (method === "template" && opts.templateId) {
    return generateFromTemplate(opts.templateId, opts);
  }
  return generateTileLayoutV3(opts.width, opts.height, opts.theme, opts.tilesetConfig, opts.seed, opts.tilesetId);
}

export { generateTileLayoutV3 };
export { generateMap };
